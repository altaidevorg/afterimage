...
@click.option("--app-name", default="banking_app", help="Target application domain name.")
@click.option("--app-desc", default="Customer banking and money transfer application.", help="App domain description.")
@click.option("-n", "--num-trajectories", default=5, type=int, help="Number of synthetic trajectories to generate.")
@click.option("-o", "--output", default="outputs/agent_trajectories.jsonl", help="Output JSONL file path.")
@click.option("--architect-model", default="gemini-3.6-flash", help="LLM Schema Architect model.")
@click.option("--teacher-model", default="gemini-3.5-flash-lite", help="ReAct Teacher Agent model.")
@click.option("--judge-model", default="gemini-3.6-flash", help="Trajectory Judge model.")
def agent_trace_command(
    ...
    if not api_key:
        click.secho("API key not found. Please set GEMINI_API_KEY or OPENAI_API_KEY environment variable.", fg="red", err=True)
        raise SystemExit(1)
    ...
    actions = [
        ...
        ToolActionSpec(
            ...
            parameters=[
                ...
                ToolParameterSpec(name="amount", type="float", description="Transfer amount"),
            ],
    ...
    async def _run():
        click.secho("Running LLM Schema Architect with static verification loop...", fg="yellow")
        await generator.register_app_domain(app_name=app_name, app_description=app_desc, actions=actions)
        click.secho(f"Generating {num_trajectories} synthetic agent trajectories...", fg="yellow")
        results = await generator.generate(num_trajectories=num_trajectories)
        click.secho(f"Successfully generated {len(results)} valid trajectories -> {output}", fg="green")
