...
def filter_and_prepare_dataset(training_file, tool_filter_config, target_dir) -> str:
    ...
    if (
        tool_filter_config is None
        or (isinstance(tool_filter_config, pd.DataFrame) and tool_filter_config.empty)
        or (isinstance(tool_filter_config, list) and not tool_filter_config)
    ):
        return merged_path
            ...
            if phase == "loading":
                ...
                yield (
                    ...
                    make_progress_display(
                        current_percent, current_remaining, spinner_idx
                    ),
                )
...
async def run_training(
    training_file, tool_filter_config=None
) -> AsyncGenerator[Tuple[str, str], None]:
    """
    ...
    try:
        ...
        try:
            prepared_dataset_path = filter_and_prepare_dataset(
                training_file, tool_filter_config, data_dir
            )
        ...
        async for status, output in _execute_training_subprocess(
            cmd, training_dir, is_dev_mode=False
        ):
            yield status, output
...
async def run_training_developer(
    ...
    try:
        ...
        async for status, output in _execute_training_subprocess(
            cmd, training_dir, is_dev_mode=True
        ):
            yield status, output
