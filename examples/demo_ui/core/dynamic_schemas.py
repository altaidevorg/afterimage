...
def create_pydantic_model_from_schema(name: str, schema: Dict[str, Any]) -> Type[BaseModel]:
    ...
    for param_name, param_info in properties.items():
        ...
        default = param_info.get("default", ...) if is_required else param_info.get("default", None)
...
def create_dynamic_tool_invocation_schema(tools: List[Union[Type[BaseModel], Dict[str, Any]]]) -> Type[BaseModel]:
    """
    ...
    AnyToolCallValidationModel = create_model(
        "DynamicAnyToolCall",
        function=(ToolUnion, Field(...))
    )
    
    # Define the top-level ToolInvocation schema
    DynamicToolInvocation = create_model(
        "DynamicToolInvocation",
        reasoning=(str, Field(description="Chain-of-thought reasoning for selecting the specific tool(s) and arguments.")),
        response=(str, Field(description="The final response to the user in natural language.")),
        tool_calls=(List[AnyToolCallValidationModel], Field(description="A list of tool calls to execute."))
    )
