...
def create_category_toggle_handler(cat_index: int):
    ...
    def toggle(
        checked: bool, current_selection: List[str], all_cat_datasets: List[List[str]]
    ):
        category_datasets = (
            all_cat_datasets[cat_index] if cat_index < len(all_cat_datasets) else []
        )
        ...
        if checked:
            ...
            if (
                len(current_selection) == len(category_datasets)
                and len(category_datasets) > 0
            ):
                return []
    ...
    with gr.Blocks() as page:
        ...
        with gr.Group(visible=True) as step_1_group:
            ...
            with gr.Row():
                with gr.Column(scale=1, elem_id="dataset-library-col"):
                    ...
                    all_cat_selects = []
                    all_cbs = []
                    initial_cat_datasets = [
                        cat_data[i][1] for i in range(MAX_CATEGORIES)
                    ]
                    ...
                    with gr.Column(elem_id="dataset-list-scroll"):
                        for i in range(MAX_CATEGORIES):
                            ...
                            if cat_name:
                                cat_select = gr.Checkbox(
                                    label=f"{cat_name} ({len(cat_ds)})", value=False
                                )
                                cb = gr.CheckboxGroup(
                                    choices=cat_ds,
                                    value=[],
                ...
                with gr.Column(scale=2):
                    ...
                    with gr.Group(visible=True) as empty_panel:
                        gr.HTML(
                            '<div style="text-align:center;padding:60px;color:#94a3b8;"><div style="font-size:16px;">Select datasets to see training overview</div></div>'
                        )
                    ...
                    with gr.Group(visible=False) as edit_panel:
                        ...
                        edit_name_input = gr.Textbox(
                            label="Dataset Name", placeholder="dataset_name.jsonl"
                        )
                        edit_category_dropdown = gr.Dropdown(
                            ...
                            + (
                                ["Uncategorized"]
                                if "Uncategorized" not in cat_names
                                else []
                            ),
                            value="Uncategorized",
        ...
        with gr.Group(visible=False) as step_2_group:
            ...
            with gr.Tabs():
                with gr.Tab("Normal Mode"):
                    ...
                    download_model_btn = gr.DownloadButton(
                        label="Download Model", visible=False
                    )
                with gr.Tab("Developer Mode"):
                    with gr.Row():
                        num_epochs = gr.Slider(
                            label="Epochs", minimum=1, maximum=10, value=3
                        )
                        batch_size = gr.Slider(
                            label="Batch Size", minimum=1, maximum=16, value=1
                        )
                    train_dev_btn = gr.Button(
                        "Start Training (Dev)", variant="primary", size="lg"
                    )
            ...
            with gr.Row():
                ...
                next_btn_2 = gr.Button(
                    "Next: Evaluate", variant="primary", interactive=False
                )
        ...
        with gr.Group(visible=False) as step_3_group:
            ...
            with gr.Row():
                ...
                next_btn_3 = gr.Button(
                    "Next: Chat", variant="primary", interactive=False
                )
        ...
        def navigate(
            current_step: int, direction: int
        ...
        next_btn_1.click(
            lambda step: navigate(step, 1), inputs=[step_state], outputs=nav_outputs
        )
        back_btn_2.click(
            lambda step: navigate(step, -1), inputs=[step_state], outputs=nav_outputs
        )
        ...
        restart_btn.click(
            lambda step: navigate(step, -step), inputs=[step_state], outputs=nav_outputs
        )
        ...
        def on_selection_change(*checkbox_values):
            ...
            if not all_selected:
                slider_updates = [
                    ...
                    for _ in range(MAX_TOOL_SLIDERS)
                ]
                container_updates = [
                    gr.update(visible=False) for _ in range(MAX_TOOL_SLIDERS)
                ]
                return (
                    [
                        NO_SELECTION_TEXT,
            ...
            paths, overview, dist, config, _, _, total_samples = on_dataset_select(
                all_selected
            )
            ...
            sorted_tools = (
                sorted(dist.items(), key=lambda x: x[1], reverse=True) if dist else []
            )
        ...
        def update_config_and_summary(*args) -> Tuple[dict, str]:
            ...
            selected = (
                min(selected_sum, total_samples) if total_samples else selected_sum
            )
            summary_text = f"**{selected} / {total_samples} samples**"

            return new_config, summary_text

        slider_inputs = tool_sliders + [
            filter_config_state,
            tool_names_state,
            total_samples_state,
        ]
        ...
        def show_edit(*checkbox_values):
            ...
            return (
                ...
                gr.update(
                    choices=all_categories, value=current_category
        ...
        def refresh_after_action():
            ...
            for i in range(MAX_CATEGORIES):
                if i < len(new_cats):
                    ...
                    results.append(
                        gr.update(
                            label=f"{cat} ({len(datasets)})", visible=True, value=False
                        )
                    )
        ...
        edit_save_btn.click(
            do_edit, inputs=[edit_target, edit_name_input, edit_category_dropdown]
        ...
        merge_save_btn.click(do_merge, inputs=[*all_cbs, merge_name_input]).then(
            refresh_after_action, outputs=refresh_outputs
        )
        ...
        split_save_btn.click(
            do_split,
            inputs=[split_target, split_tools_state, split_base_name, split_num_groups]
            + split_tool_dropdowns,
        ).then(refresh_after_action, outputs=refresh_outputs)
        ...
        def show_delete(*checkbox_values):
            ...
            info = (
                f"**Delete {len(all_selected)} dataset(s)?**\n\nThis cannot be undone.\n\n"
                + "\n".join(f"- {s}" for s in all_selected)
            )

            paths = [
                get_dataset_path(extract_filename_from_label(label))
        ...
        delete_cancel_btn.click(
            cancel_delete_fn, outputs=[delete_panel, overview_panel]
        )

        # Refresh datasets button
        refresh_datasets_btn.click(refresh_after_action, outputs=refresh_outputs)
        ...
        train_btn.click(
            ...
            outputs=[
                ...
                download_model_btn,
                download_status_dev,
                download_model_dev_btn,
            ],
        ).then(lambda: gr.update(interactive=True), outputs=[next_btn_2])

        download_model_btn.click(
            set_download_loading, outputs=[download_model_btn]
        ).then(generate_model_zip, outputs=[download_model_btn])
        download_model_dev_btn.click(
            set_download_loading, outputs=[download_model_dev_btn]
        ).then(generate_model_zip, outputs=[download_model_dev_btn])
        ...
        if chat_fn:
            send_btn.click(
                chat_fn, inputs=[msg_input, chatbot], outputs=[msg_input, chatbot]
            )
            msg_input.submit(
                chat_fn, inputs=[msg_input, chatbot], outputs=[msg_input, chatbot]
            )
