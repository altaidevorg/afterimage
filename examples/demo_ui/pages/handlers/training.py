...
def merge_datasets(selected: List[str] | None, new_name: str):
    ...
    return gr.Info(
        f"Merged {len(selected)} datasets into {filename} ({total_samples} samples)"
    )
...
def split_dataset(
    dataset_path: str, tool_groups: dict[str, list[str]], base_name: str | None = None
) -> List[str]:
    """
...
def create_analyze_handler(analyze_fn):
    ...
    def on_analyze(training_file):
        ...
        test_pct_of_total = (
            stats["test_samples"] / stats["total_samples"] * 100
            if stats["total_samples"] > 0
            else 0
        )
        test_pct_of_tool = (
            stats["test_samples"] / stats["tool_call_samples"] * 100
            if stats["tool_call_samples"] > 0
            else 0
        )

        total_samples = (
            stats["total_samples"] if stats["total_samples"] > 0 else 1
...
def on_train_complete(status, logs):
    ...
    if not status:
        return (
            gr.update(),
            gr.update(),  # eval_btn, eval_status
            gr.update(),
            gr.update(),  # download_status, download_model_btn
            gr.update(),
            gr.update(),  # download_status_dev, download_model_dev_btn
        )
    ...
    if "Complete" in status:
        ...
        download_status_update = gr.update(
            value="Training complete. Click to download.", visible=True
        )
        download_status_dev_update = gr.update(
            value="Training complete. Click to download.", visible=True
        )

        # Make buttons visible and interactive with size label
        download_btn_update = gr.update(
            visible=True, value=None, label=label, interactive=True
        )
        download_dev_btn_update = gr.update(
            visible=True, value=None, label=label, interactive=True
        )
        ...
        return (
            ...
            download_btn_update,
            download_status_dev_update,
            download_dev_btn_update,
        )
