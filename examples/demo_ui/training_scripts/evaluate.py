        ...
        if "<function=" in response:
            ...
            for match in re.finditer(pattern, response):
                ...
                if arguments is not None:
                    ...
                    print(
                        f"  [WARNING] Failed to parse args for {tool_name}: {args_str[:100]}"
                    )
...
def compare_tool_calls(expected, predicted):
    ...
    if correct_tools == exp_count and correct_args == exp_count:
        ...
        return "correct_tools_wrong_args", {
...
def evaluate_model(model_path=None):
    ...
    tools = (
        load_tools_schema(config.TOOLS_FILE)
    ...
    for idx, item in enumerate(test_data, 1):
        ...
        # Generate prediction
        predicted_tool_calls = generate_tool_calls(model, tokenizer, messages, tools)

        # Compare
        category, details = compare_tool_calls(
            expected_tool_calls, predicted_tool_calls
        )
