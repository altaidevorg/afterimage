...
async def main():
    ...
    storage = JSONLStorage(personas_path="generated_personas.jsonl")
    ...
    personas = await persona_gen.generate_async(single_text)
    ...
    doc_provider = InMemoryDocumentProvider(documents=docs)
    await persona_gen.generate_for_documents(doc_provider, max_concurrency=2)
    print(f"Batch generation complete. Personas saved to {storage.personas_path}")
